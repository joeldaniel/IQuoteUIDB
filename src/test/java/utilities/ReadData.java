...
public class ReadData extends Testbase{
	...
	public HashMap<String, String> CPGraphBindStitch(int Estimateid, String componentorder, String CharacteristicDescp) throws ClassNotFoundException, SQLException, IOException

	{
		CharCPGraphBindStitch.clear();
		fis = new FileInputStream(
				System.getProperty("user.dir") + "\\src\\test\\resources\\properties\\CPGraphBindStitch.properties");
		Query.load(fis);
		String Query1=Query.getProperty("Query");
		// opening database connection to MySQL server
		DBUtil iqdb=new DBUtil(DBurl, DBuser, DBpass);
		rs = iqdb.RunQuery(Query1.replace("##Estimate##", Config.getProperty("EstimateIDs").replace("##ComponentOrder##", componentorder)));
		try
		{
			//loop
			while(rs.next())
